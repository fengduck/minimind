...
def train_epoch(epoch, wandb):
    ...
    for step, batch in enumerate(train_loader):
        ...
        # GradScaler 的核心作用
        # 在反向传播前，先把 loss 放大 N 倍（比如 1024 倍）再 backward，之后再把梯度缩回来
        scaler.scale(loss).backward() # 等价于 (loss * scale_factor).backward()

        if (step + 1) % args.accumulation_steps == 0:
            # 这一步是把刚刚放大的梯度除回来（还原到正常尺度）
            scaler.unscale_(optimizer)
            # 这一步其实和混合精度无关。
            # 它是为了防止梯度爆炸（数值太大导致训练不稳定），
            # 会在每次更新前将梯度的 L2 范数裁剪到某个上限。
            #
